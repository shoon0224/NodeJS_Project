    ...
    conn.query(
      ...
      function (err, row) {
        ...
        if (row) {
          ...
          res.write(
            "<script>alert('장난감이 등록되었습니다.');location.href='/';</script>"
          );
        } else {
          res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
          res.write(
            "<script>alert('장난감 등록이 완료되지 않았습니다.');history.back();</script>"
          );
        }
      }
    );
...
router.get('/modify/:toyId', function (req, res, next) {
  ...
  pool.getConnection(function (err, conn) {
    ...
    var sql =
      "select *,date_format(endTime,'%y-%m-%d %T') AS endTime from toy where toyId=?";
...
router.post('/modify/:toyId', upload.single('photo'), function (
  req,
  res,
  next
) {
  ...
  pool.getConnection(function (err, conn) {
    ...
    if (req.file != null) {
      ...
      var imgurl = 'images/' + req.file.originalname;
      var sql =
        'update toy set toyName=?, toyRprice=?, saleReason=?, toyExp=?, toyPic=? where toyId=?';
      ...
      conn.query(
        ...
        function (err, result) {
          ...
          if (result) {
            res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
            res.write(
              "<script>alert('정보가 수정되었습니다.');location.href='/';</script>"
            );
          } else {
            res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
            res.write(
              "<script>alert('정보 수정이 완료되지 않았습니다.');history.back();</script>"
            );
          }
        }
      );
      ...
      console.log('img');
      var sql =
        'update toy set toyName=?, toyRprice=?, saleReason=?, toyExp=? where toyId=?';
      var toyRprice = parseInt(req.body.toyRprice);
      conn.query(
        sql,
        [
  ...
  pool.getConnection((err, conn) => {
    ...
    conn.query(sql, [toyId], function (err, result) {
      ...
      if (result) {
        ...
        res.writeHead(200, { 'content-type': 'text/html; charset=utf-8' });
        res.write(
          "<script>alert('삭제가 되지 않았습니다.');history.back();</script>"
        );
      }
    });
  });
...
router.get('/detail/:toyId', function (req, res, next) {
  ...
  pool.getConnection((err, conn) => {
    ...
    console.log('DB Connection');
    var sql =
      "select toyId,toyName,toyPic,bidState,currentCoin,saleReason,bidNum,toyExp,date_format(endTime,'%y-%m-%d %T') AS endTime from toy where toyId = ?";
